...
ENV DEBIAN_FRONTEND=noninteractive \
    LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 \
    SETUPTOOLS_USE_DISTUTILS=stdlib \
    # Forzar PRIME Render Offload (dGPU en portátil híbrido)
    __NV_PRIME_RENDER_OFFLOAD=1 \
    __GLX_VENDOR_LIBRARY_NAME=nvidia \
    __VK_LAYER_NV_optimus=NVIDIA_only \
    # Evitar vsync
    __GL_SYNC_TO_VBLANK=0 \
    # Evitar MIT-SHM en X11 dentro de Docker
    QT_X11_NO_MITSHM=1

# Herramientas base + glxinfo
RUN apt-get update && apt-get install -y --no-install-recommends \
    locales curl gnupg lsb-release ca-certificates software-properties-common \
    build-essential python3-dev python3-pip python3-distutils \
    libglvnd0 libgl1 libglx0 libegl1 \
    libxext6 libx11-6 libxrandr2 libxinerama1 libxi6 \
    libglib2.0-0 mesa-utils \
    libqt6gui6 libqt6widgets6 libqt6core6 qt6-base-dev && \
    locale-gen en_US.UTF-8 && add-apt-repository -y universe && \
    rm -rf /var/lib/apt/lists/*
...
# Python extra
RUN python3 -m pip install --no-cache-dir --upgrade pip setuptools~=69.5 wheel packaging && \
    python3 -m pip install --no-cache-dir "numpy<1.24" "PySide6~=6.5" Cython~=0.29 utm pyproj

# Workspace
WORKDIR /ros2_ws
COPY src ./src

# Asegura __init__.py en paquete Python 'car' si falta
RUN bash -lc "set -e; \
  if [ ! -f src/car/car/__init__.py ]; then \
    install -d src/car/car && touch src/car/car/__init__.py; \
  fi"
